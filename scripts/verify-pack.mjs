 ...
 *
 * feature-flags-kit has ZERO runtime dependencies, so the throwaway
 * consumer installs only the tarball.
 *
...
const EXPECTED_VALUES = ['FEATURE_FLAGS_SCHEMA_SQL'];
...
const workDir = mkdtempSync(join(tmpdir(), 'feature-flags-verify-'));
let tarballPath;
...
try {
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: 'feature-flags-kit-consumer', version: '1.0.0', private: true }, null, 2),
  );
  ...
  const functionNames = JSON.stringify(EXPECTED_FUNCTIONS);
  const valueNames = JSON.stringify(EXPECTED_VALUES);

  // 2. CommonJS require smoke.
  const cjsSmoke = `
    const mod = require('${pkg.name}');
    const missingFns = ${functionNames}.filter((n) => typeof mod[n] !== 'function');
    const missingVals = ${valueNames}.filter((n) => mod[n] === undefined);
    if (missingFns.length || missingVals.length) {
      console.error('CJS missing exports: ' + [...missingFns, ...missingVals].join(', '));
      process.exit(2);
    }
    console.log('CJS OK');
  `;
  ...
  const esmSmoke = `
    ...
    const missingVals = ${valueNames}.filter((n) => mod[n] === undefined);
    if (missingFns.length || missingVals.length) {
      console.error('ESM missing: ' + [...missingFns, ...missingVals].join(', '));
      process.exit(5);
    }
    console.log('ESM OK');
  `;
